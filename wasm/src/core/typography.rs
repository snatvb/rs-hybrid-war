...
use super::geometry::{Point, Polyline};
    ...
    pub fn new(height: f64) -> Self {
        ...
        Font {
            ...
            glyphs: master::GLYPHS
                .iter()
                .map(|(c, d)| (*c, path::Data(d).to_polylines(scale)))
                .collect(),
        }
    }
...
mod path {
    use super::super::geometry::{Point, Polyline};
                ...
                for command in commands {
                    match *command {
                        ...
                        Command::C(x2, y2, x3, y3, x4, y4) => {
                            if let Some(p1) = polyline.last().cloned() {
                        ...
                        Command::H(x) => {
                            if let Some(mut point) = polyline.last().cloned() {
                        ...
                        Command::Z => {
                            if let Some(point) = polyline.first().cloned() {
                                polyline.push(point)
                            }
                        }
                    }
